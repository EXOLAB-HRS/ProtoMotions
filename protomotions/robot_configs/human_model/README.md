...
- `human_model_v2/profiles/full_joint_strength_candidate.json`: 상지·몸통·목·hip 비 sagittal·subtalar·toe의 자세별 양방향 근력 표. `configure_full_joint_strength_candidate(robot_cfg)`로만 명시 적용하며 기본 v2 profile에는 자동 적용하지 않는다. 몸통/목 직렬축은 분절각 합을 사용하고 shoulder-girdle proxy는 독립 능동 구동을 0으로 둔다. 수치 screening 후보이며 보행·독립 피험자 confirmation 전에는 채택 상태가 아니다.

...
def wrist_damping_assay(model):
    ...
    params = json.loads(Path(__file__).with_name('candidate_parameters.json').read_text())['wrist_damping']
...
def isaaclab_kinematic_audit(sim, initial, model):
    ...
    from protomotions.simulator.base_simulator.simulator_state import ResetState, StateConversion
    source=Path(__file__).resolve().parents[2]/'data/assets/mjcf/smpl_humanoid.xml'
...
def run(output, stage, seed, features, backend='mujoco'):
    ...
    result = {"implementation_id":model.profile_id,"features":features,"seed":seed,"stage":stage,
              ...
              "contract_sha256":sha256(Path(__file__).with_name("validation_contract.json")),
              "candidate_parameters_sha256":sha256(Path(__file__).with_name("candidate_parameters.json")),
    ...
    contract = json.loads(Path(__file__).with_name('validation_contract.json').read_text())
...
def finalize_all_direction_videos(output, sources_path):
    ...
    root=Path(__file__).resolve().parents[4];deliverables=output/'deliverables';videos=root/'videos'
...
if __name__ == '__main__':
    ...
    if args.validation_video:
        if args.video_worker:
            ...
            source_files=list(Path(__file__).parent.rglob('*.py'))+list(Path(__file__).parent.glob('*.json'))

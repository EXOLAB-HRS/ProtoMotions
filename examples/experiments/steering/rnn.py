...
"""
Steering Environment Configuration with AMP — RECURRENT (GRU) actor
====================================================================

Identical to steering/mlp.py except the actor's mu network is a RecurrentMLPWithConcat:
a GRU unrolls the recent motor-state history (historical_max_coords_obs, 8 steps) into a
hidden state each timestep, which is combined with the current observation to emit the
action. This models the ④ MotionDriver as a recurrent dynamical system (Churchland/Shenoy
M1). Critic/discriminator stay feedforward MLPs — only the policy core is recurrent.
"""
...
def env_config(robot_cfg: RobotConfig, args: argparse.Namespace) -> EnvConfig:
    """Build environment configuration (training defaults). Identical to steering/mlp.py."""
    ...
    env_cfg = EnvConfig(
        ...
        num_state_history_steps=8,  # Historical obs; also the GRU sequence length
...
def agent_config(
    ...
    # Actor mu = GRU-over-history recurrent core (the only change vs steering/mlp.py).
    actor_config = PPOActorConfig(
        num_out=robot_config.kinematic_info.num_dofs,
        actor_logstd=-2.9,
        in_keys=["max_coords_obs", "steering", "historical_max_coords_obs"],
        mu_key="actor_trunk_out",
        mu_model=RecurrentMLPWithConcatConfig(
            in_keys=["max_coords_obs", "steering", "historical_max_coords_obs"],
            normalize_obs=True,
            norm_clamp_value=5,
            out_keys=["actor_trunk_out"],
            num_out=robot_config.number_of_actions,
            history_key="historical_max_coords_obs",
            num_history_steps=8,  # must match env_cfg.num_state_history_steps
            gru_hidden_size=256,
            gru_num_layers=1,
            layers=[  # MLP head after the GRU
    ...
    reference_obs_components = {
        "historical_max_coords_obs": MdpComponent(
            ...
            dynamic_vars={},
            static_params={"num_state_history_steps": 8},
        ),
    }

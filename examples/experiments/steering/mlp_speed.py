...
"""
Steering Environment Configuration with AMP — sharpened speed tracking (④ M1 MotionDriver)
==========================================================================================

Near-copy of steering/mlp.py. The ONLY functional change is the heading reward's
``vel_err_scale``: 0.25 (mlp.py default) -> 1.5 here. The soft default lets the AMP-shaped
policy quantize to a few reference gaits instead of tracking the commanded speed continuously
(a 0.4 m/s undershoot costs only ~4% reward at 0.25). Raising it to 1.5 makes speed error a
real gradient so ④ tracks 0.5-1.5 m/s without the gait-jump discontinuity. See
docs/2026-07-04-p2-execution-log.md "Speed-tracking re-measurement" for the diagnosis and the
retrain target (track% >= ~90% across 0.5-1.5, upright >= 0.85). task_reward_w is kept at 0.5
for this first run to isolate the vel_err_scale effect; bump it later only if AMP still pulls.
"""
...
def env_config(robot_cfg: RobotConfig, args: argparse.Namespace) -> EnvConfig:
    ...
    reward_components = {
        ...
        # vel_err_scale=1.5 (vs the kernel default 0.25) sharpens speed tracking; see module docstring.
        "heading_rew": MdpComponent(
            ...
            static_params={"weight": 1.0, "vel_err_scale": 1.5},
        ),
    }
...
def agent_config(
    ...
    reference_obs_components = {
        "historical_max_coords_obs": MdpComponent(
            ...
            static_params={"num_state_history_steps": 8},  # Matches env_cfg.num_state_history_steps (buffer depth); history_steps=None uses all 8
        ),
    }
